(ns helping-hands.auth.jwt
  ...
  (:import [com.nimbusds.jose
            EncryptionMethod JWEAlgorithm
            JWSAlgorithm JWEDecrypter JWEEncrypter
...
(defn get-secret
  ...
  ([kgen]
   ;; must be created iff the key hasn't been created earler.
   ;; Create once and persist in an external database
   (.generateKey kgen)))

(defn get-secret-jwk
  "Generates a new JSON Web Key (JWK)"
  [{:keys [khash kgen alg] :as enc-impl} secret-key]
  ;; must be created iff the key hasn't been created earler.
  ;; Create once and persist in an external database
  (.. (OctetSequenceKey$Builder. secret-key)
...
(defn enckey->secret
  "Convert JSON Web Key (JWK) to the secret key"
...
(defn- create-payload
  ...
  (let [ts     (System/currentTimeMillis)
        claims (.. (JWTClaimsSet$Builder.)
                   ...
                   (audience "https://www.github.com/liverm0r")
                   (issueTime (Date. ts))
