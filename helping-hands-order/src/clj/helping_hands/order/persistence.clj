...
(defprotocol OrderDB
  ...
  (upsert [this id service provider consumer cost start end rating status])
  (entity [this id flds])
  (orders [this uid flds])
  (delete [this id]))

;; Datomic Adapter Inplementation for Order Port
(defn- get-entity-id [conn id]
  (-> (d/q '[:find ?e
             :in $ ?id
             :where [?e :order/id ?id]]
           (d/db conn)
...
(defn- get-entity-uid [conn uid]
  (->> (d/q '[:find ?e
              :in $ ?id
              :where [?e :order/consumer ?id]]
            (d/db conn)
            (str uid))
       (into [])
       flatten))

(defn- get-all-entities [conn uid]
  (let [eids (get-entity-uid conn uid)]
    (map #(->> (d/entity (d/db conn) %) seq (into {}))
         eids)))
...
(defn create-order-database
  "Creates a order database and returns the cvonnection"
  [d]
